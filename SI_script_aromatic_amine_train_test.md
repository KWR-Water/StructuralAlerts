...
spectra with an aromatic amine structural alert.
...
``` r
# select data type (neutral losses or fragments)
data.type <- "neutrallosses"
#data.type <- "fragments"

# load corresponding data set
if (data.type == "neutrallosses") {
  data <- read.csv(file = "data\\MassBankEU_202206_neutrallosses_pos_hrms.csv")
} else if (data.type == "fragments") {
  data <- read.csv(file = "data\\MassBankEU_202206_fragments_pos_hrms.csv")
}

# Count occurrence of alerts
alert.count <- data %>%
  count(alert)

# select spectra of molecules with the aromatic amine alert, some might contain other alerts as well.
...
# select spectra of molecules without the aromatic amine alert
...
# combine both data sets (spectra with aromatic amine alert, and random subset of spectra without the aromatic amine alert)
